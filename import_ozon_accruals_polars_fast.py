...
* **Excel and CSV support**: The function will automatically detect
  `.xlsx` files and use Polars' Excel reader.  CSV files are read
  lazily using Polars' scan API.  This implementation does not
  depend on pandas, satisfying environments where only Polars is
  available.
...
def import_ozon_accruals_polars(
    ...
    *,
    ...
    """
    # Initialise stats variables.  ``rows_read`` will be updated after
    # loading the file via Polars (either CSV or Excel).  We avoid
    # counting rows up front using pandas to honour the user request
    # to use Polars exclusively.
    ...
    if lower.endswith(".csv"):
        # -----------------------------------------------------------------
        # CSV handling via Polars' lazy API
        # -----------------------------------------------------------------
        lf = pl.scan_csv(
            file_path,
            separator=';',
        ...
        if cols and cols[0].startswith("\ufeff"):
            ...
            cols = lf.collect_schema().names()
        # If stats requested, compute row count for CSV
        if return_stats and rows_read is None:
            try:
                count_df = lf.select(pl.count()).collect()
        ...
        # Build date column for filtering.  Always cast to string first
        # and let Polars infer the date format with format=None.  Then
        # extract the date component.  This approach handles mixed
        # formats like "2025/8/1" and "2025-08-01" without requiring
        # pandas.  We use strict=False so that unparseable strings
        # become nulls, which are filtered out by date filters.
        fdate = (
            pl.col(date_field)
        ...
        # Apply date filters lazily on the derived date column
        if include_dates_parsed:
            ...
            lf = lf.filter((pl.col("_fdate") >= start_dt) & (pl.col("_fdate") <= end_dt))
        lf = lf.drop("_fdate")
        # Collect the filtered data into a DataFrame
        df = lf.collect(streaming=True)
    else:
        # -----------------------------------------------------------------
        # Excel handling via Polars (no pandas)
        # -----------------------------------------------------------------
        # Read the specified sheet with Polars.  Use sheet_id for
        # numeric sheet_name, otherwise sheet_name.  pl.read_excel
        # automatically uses openpyxl under the hood.
        ...
        # Load the Excel file into a Polars DataFrame
        df = pl.read_excel(file_path, **read_kwargs)

        # Remove BOM from first column if present
        cols = df.columns
        if cols and isinstance(cols[0], str) and cols[0].startswith("\ufeff"):
            df = df.rename({cols[0]: cols[0].lstrip("\ufeff")})
            cols = df.columns

        # If stats requested and rows_read not already set, set rows_read
        if return_stats and rows_read is None:
            rows_read = df.height
        ...
        if date_field not in cols:
            ...
            if alt is None:
                raise KeyError(
                    f"date_field '{date_field}' 不在列中；现有列: {cols}"
                )
            date_field = alt

        # Build date column for filtering.  Cast to Utf8 before
        # parsing to support varied formats.  Use ``pl.Date`` as the
        # target dtype so that Polars produces a date directly.  This
        # avoids needing pandas to parse Excel date formats.
        df = df.with_columns(
            pl.col(date_field)
            .cast(pl.Utf8, strict=False)
            .str.strip_chars()
        ...
        if include_dates_parsed:
            df = df.filter(pl.col("_fdate").is_in(include_dates_parsed))
        elif start_dt and end_dt:
            df = df.filter((pl.col("_fdate") >= start_dt) & (pl.col("_fdate") <= end_dt))
        elif days_back and days_back > 0:
            df = df.filter((pl.col("_fdate") >= start_dt) & (pl.col("_fdate") <= end_dt))
