...
# =============================================================================
# 主流程：生成最终分组的 Polars DataFrame（按日时效）
# =============================================================================
def build_grouped_df(
    ...
    avg_delivery_hours_by_date: Union[List[int], Dict[str, int]],  # ← 新参数：按日时效
    ...
    # —— 构建“日期 → 当日时效小时”的映射，并生成 DataFrame 以便 join —— #
    ...
    if df_orders.select(pl.col("当日时效小时").is_null().any()).item():
        raise ValueError("有订单行未匹配到当日时效小时，请检查日期与传入的 avg_delivery_hours_by_date。")

    # —— 2) accruals：按“Дата принятия заказа ...” 的日期 ∈ 所选日期 —— #
    accr = fetch_accruals_by_acceptance_dates(
        coll_accr,
        acceptance_dates_ymd=dates,
        charge_type="Доставка покупателю",
        acceptance_field="Дата принятия заказа в обработку или оказания услуги",
    )
    ...
    def fallback_fee_struct(s):
        return compute_fallback_logistics_fee(
            ...
            avg_delivery_hours=int(s["当日时效小时"]),  # ← 按行取对应日期的时效
            order_date_ymd=s["日期"],
            rule_mode=rule_mode,
...
if __name__ == "__main__":
    ...
    # 所选日期（可乱序/不同分隔，函数会规范化并排序）
    selected_dates = ["2025/8/21", "2025-08-25"]

    # 每日时效：可以用 list（按规范化后排序的日期对应顺序）……
    # 例如：2025-08-21 -> 30 小时，2025-08-25 -> 32 小时
    hours_list = [30, 32]

    # ……或用 dict（key 会被规范化到 YYYY-MM-DD）
    # hours_dict = {"2025/8/21": 30, "2025-08-25": 32}
    ...
    df_out = build_grouped_df(
        ...
        avg_delivery_hours_by_date=hours_list,   # 或者传 hours_dict
        sku_volume_df=sku_volume_df,
        rule_mode="auto",                        # "auto"|"old"|"new"
        rule_boundary_date_ymd="2025-09-01",
        # 可选自定义：
        # hour_rules=your_hour_rules_dict,
        # light_small_ids=your_light_small_ids_set,
        # new_base_fee_brackets=your_brackets_list,
        light_small_fee_per_unit=11.0,
    )
    print(df_out)
