...
def get_collection(cfg: MongoConfig):
    ...
    coll.create_index([("campaignId", 1), ("date", 1), ("sku", 1)], background=True)
    coll.create_index([("createdAt", 1)], background=True)
    return coll
...
def upsert_docs(cfg: MongoConfig, docs: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    ops = []
    for d in docs:
        key = md5_of_fields(d.get("campaignId"), d.get("date"), d.get("sku"))
        d["dedup_md5"] = key
