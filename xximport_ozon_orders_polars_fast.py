...
def import_ozon_orders_polars(
    ...
    *,
    ...
    # Apply MD5 using a Python lambda.  This runs in Rust side via
    # ``apply`` but falls back to Python for hashing.  The overhead is
    # negligible compared to I/O.
    # df = df.with_columns(
    #     pl.col("__hash_key").apply(lambda s: hashlib.md5(s.encode("utf-8")).hexdigest()).alias(hash_field)
    # )
    # df = df.drop("__hash_key")
    df = df.with_columns(
    pl.col("__hash_key").map_elements(
...
if __name__ == "__main__":
    # Example usage
    n = import_ozon_orders_polars("orders.csv", days_back=10)
    print(f"Inserted {n} new documents")
